...
A = input("What is A? (y-intercept from calibration plot)")
B = input("What is B? (slope from calibration plot)")
...
Channel = np.array(V1)
Counts1 = np.array(W1)
Energy = Channel/B - A/B
#Smoothing data for peak finding using the Gaussian averaging function defined above

smooth1 = smoothListGaussian(Counts1,degree=5)

smooth2 = np.append(smooth1, [0,0,0,0,0])
smoothed = np.insert(smooth2, 0, [0,0,0,0])
#smooth4 = np.insert(smooth3, 0, 0)
#smooth5 = np.insert(smooth4, 0, 0)
#smoothed = np.insert(smooth5, 0, 0)
...
series = smoothed
oldmaxtab, mintab = peakdet(series, Delta)
maxtab1 = oldmaxtab[:,0]
maxtab2 = oldmaxtab[:,1]
calimaxtab = maxtab1/B - A/B
maxtab = np.rint(calimaxtab) 
...
axes1.plot(Energy, Counts1, color="green", lw=0.5, label="Raw")
axes1.plot(Energy, smoothed, color="red", lw=1, label="Smoothed")
axes1.scatter(maxtab, maxtab2, color='black')
axes1.set_title(Source, fontsize=20)
axes1.set_xlabel("Energy keV", fontsize=20)
...
print("Peaks at:")
print("[Energy keV,Count]")
print(np.column_stack((maxtab, maxtab2)))
plt.show()
...
if Manual_Params=="y":
    while True:
        ...
        EnergyMin = input("Enter minimum Energy:")
        EnergyMax = input("Enter maximum Energy:")
...
if Gaussian_Fit == "y":
    while True:
        ...
        if Gaussian == "y":
            m0 = input("enter energy for peak of interest (from peak find output above):")
            x0 = np.rint(m0*B + A).astype(int)
            w = input("enter width for region of interest (best guess):")
            n = np.rint(w*B).astype(int)
            itemindex = np.where(maxtab == m0)
            P0 = float(maxtab[itemindex[0]])

            x = Energy[x0 - n/2:x0 + n/2]
            y = Counts1[x0 - n/2:x0 + n/2]
            sigma_Counts = np.sqrt(Counts1[x0 - n/2:x0 + n/2])
            s0 = np.sqrt(sum((x-x0)**2)/(n-1))
            
            #Curve Fitting Function
            popt, pcov = curve_fit(gauss_function, x, y, p0=[P0, m0, s0],sigma=sigma_Counts)
            ...
            axes1.plot(Energy, Counts1, color="green", lw=0.5, label="Source Data")
            axes1.plot(Energy, smoothed, color="red", lw=0.5, label="Smoothed Data")
            ...
            axes3.set_xlabel('Energy',fontsize=15)
            ...
            print("errors in fit parameters:[P,mean,std_dev]")
            ...
            if Gaussian_1 == "y":
                ...
                #Curve Fitting Function
                popt, pcov = curve_fit(firstorderpoly_gauss_function, x, y, p0=[a0, b0, P0, m0, s0], sigma=sigma_Counts)
                ...
                axes3.set_xlabel('Energy', fontsize=15)
                axes3.set_ylabel('residuals', fontsize=15)
                axes3.set_xlim([popt[3] - (n / 2), popt[3] + (n / 2)])
                axes3.set_ylim(-residplot, residplot)
                ...
                if Gaussian_2 == "y":
                    ...
                    #Curve Fitting function
                    popt, pcov = curve_fit(secondorderpoly_gauss_function, x, y, p0=[a0, b0, c0, P0, m0, s0], sigma=sigma_Counts)
                    ...
                    axes3.errorbar(x, resids, yerr=sigma_Counts, ecolor="black", fmt="ro")
                    axes3.axhline(color="gray", zorder=-1)
                    axes3.set_xlabel('Energy keV', fontsize=15)
                    axes3.set_ylabel('residuals', fontsize=15)
                    ...
                    if Gaussian_3 == "y":
                        ...
                        #Curve Fitting Function
                        popt, pcov = curve_fit(thirdorderpoly_gauss_function, x, y, p0=[a0, b0, c0, d0, P0, m0, s0],sigma=sigma_Counts)
